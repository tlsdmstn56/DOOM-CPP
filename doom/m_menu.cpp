...
void M_QuickLoad()
{
    ...
    if (quickSaveSlot < 0)
    {
	M_StartMessage(QSAVESPOT, (void*)NULL, false);
	return;
    }
...
void M_VerifyNightmare(int ch)
{
    ...
    G_DeferedInitNew(static_cast<Skill>(nightmare),epi+1,1);
    M_ClearMenus ();
}
...
void M_ChooseSkill(int choice)
{
    ...
    G_DeferedInitNew(static_cast<Skill>(choice),epi+1,1);
    M_ClearMenus ();
}
  ...
  bool	input )
{
    ...
    messageToPrint = 1;
    messageString = const_cast<char*>(string);
...
bool M_Responder (event_t* ev)
{
    ...
    if (ev->type == EventType::Joystick && joywait < I_GetTime())
    {
	if (ev->data3 == -1)
	{
	    ch = KEY_UPARROW;
    ...
    {
	if (ev->type == EventType::Mouse && mousewait < I_GetTime())
	{
	    mousey += ev->data3;
	    if (mousey < lasty-30)
	    {
