...
static patch_t*  keys[static_cast<int>(Card::NUMCARDS)]; 
...
// used for evil grin
static bool oldweaponsowned[static_cast<int>(WeaponType::NUMWEAPONS)]; 
...
ST_Responder (event_t* ev)
{
 ...
 for (i=0;i<static_cast<int>(WeaponType::NUMWEAPONS);i++)
   plyr->weaponowned[static_cast<int>(i)] = true;
 ...
 if (cht_CheckCheat(&cheat_powerup[i], ev->data1))
 {
   if (!plyr->powers[i])
     P_GivePower( plyr, i);
   else if (i!=INT(PowerType::pw_strength))
     plyr->powers[i] = 1;
   else
     plyr->powers[i] = 0;
 ...
 plyr->message = STSTR_BEHOLD;
      ...
      {
 plyr->weaponowned[static_cast<int>(WeaponType::wp_chainsaw)] = true;
 plyr->powers[INT(PowerType::pw_invulnerability)] = true;
...
void ST_updateFaceWidget()
{
 ...
 if (plyr->bonuscount)
 {
     ...
     for (i=0;i<static_cast<int>(WeaponType::NUMWEAPONS);i++)
     {
  if (oldweaponsowned[i] != plyr->weaponowned[static_cast<int>(i)])
  {
      doevilgrin = true;
      oldweaponsowned[i] = plyr->weaponowned[static_cast<int>(i)];
  }
     }
     if (doevilgrin) 
     {
 ...
 // invulnerability
 if ((plyr->cheats & CF_GODMODE)
     || plyr->powers[INT(PowerType::pw_invulnerability)])
 {
     priority = 4;
...
void ST_updateWidgets()
{
    ...
    //  {
    if (weaponinfo[static_cast<int>(plyr->readyweapon)].ammo == AmmoType::am_noammo)
 ...
 w_ready.num = &plyr->ammo[INT(weaponinfo[INT(plyr->readyweapon)].ammo)];
    ...
    // static int dir=-1;
    // if (!(tic&15))
    //   plyr->ammo[weaponinfo[static_cast<int>(plyr->readyweapon)].ammo]+=dir;
    // if (plyr->ammo[weaponinfo[static_cast<int>(plyr->readyweapon)].ammo] == -100)
    //   dir = 1;
    // tic++;
    // }
    w_ready.data = static_cast<int>(plyr->readyweapon);
...
void ST_doPaletteStuff()
{
    ...
    if (plyr->powers[INT(PowerType::pw_strength)])
    {
 // slowly fade the berzerk out
   bzc = 12 - (plyr->powers[INT(PowerType::pw_strength)]>>6);
 ...
 palette += STARTBONUSPALS;
    ...
    else if ( plyr->powers[INT(PowerType::pw_ironfeet)] > 4*32
       || plyr->powers[INT(PowerType::pw_ironfeet)]&8)
...
void ST_createWidgets()
{
    ...
    ST_AMMOY,
    tallnum,
    &plyr->ammo[INT(weaponinfo[INT(plyr->readyweapon)].ammo)],
    &st_statusbaron,
    ST_AMMOWIDTH );
 ...
 STlib_initMultIcon(&w_arms[i],
      ...
      arms[i], (int *) &plyr->weaponowned[static_cast<int>(i+1)],
      &st_armson);
    }
