...
{ 
  ...
  || ( static_cast<int>(gamemode) == static_cast<int>(GameMission::pack_tnt) )
  || ( static_cast<int>(gamemode) == static_cast<int>(GameMission::pack_plut) ) )
    {
...
{ 
    ...
    p->health = MAXHEALTH; 
    p->readyweapon = p->pendingweapon = WeaponType::wp_pistol; 
    p->weaponowned[static_cast<int>(WeaponType::wp_fist)] = true; 
    p->weaponowned[static_cast<int>(WeaponType::wp_pistol)] = true; 
    p->ammo[static_cast<int>(AmmoType::am_clip)] = 50; 
...
{ 
 ...
 sprintf (name,SAVEGAMENAME"%d.dsg",savegameslot); 
    ...
    *save_p++ = static_cast<int>(gameskill); 
    *save_p++ = gameepisode; 
    *save_p++ = gamemap; 
    for (i=0 ; i<MAXPLAYERS ; i++) 
...
{ 
 ...
 episode = 4;
    ...
    {
      if (episode > 1) 
    episode = 1; // only start episode 1 on GameMode::shareware
    }  
    else
    {
      if (episode > 3)
 ...
 map = 1;
  ...
  && ( gamemode != GameMode::commercial) )
    ...
    if (skill == Skill::Nightmare || respawnparm )
 respawnmonsters = true;
    else
 respawnmonsters = false;
  
    if (fastparm || (skill == Skill::Nightmare && gameskill != Skill::Nightmare) )
    { 
 for (i=S_SARG_RUN1 ; i<=S_SARG_PAIN2 ; i++) 
     states[i].tics >>= 1; 
 mobjinfo[static_cast<int>(MObjType::MT_BRUISERSHOT)].speed = 20*FRACUNIT; 
 mobjinfo[static_cast<int>(MObjType::MT_HEADSHOT)].speed = 20*FRACUNIT; 
 mobjinfo[static_cast<int>(MObjType::MT_TROOPSHOT)].speed = 20*FRACUNIT; 
    } 
    else if (skill != Skill::Nightmare && gameskill == Skill::Nightmare) 
    { 
 for (i=S_SARG_RUN1 ; i<=S_SARG_PAIN2 ; i++) 
     states[i].tics <<= 1; 
 mobjinfo[static_cast<int>(MObjType::MT_BRUISERSHOT)].speed = 15*FRACUNIT; 
 mobjinfo[static_cast<int>(MObjType::MT_HEADSHOT)].speed = 10*FRACUNIT; 
 mobjinfo[static_cast<int>(MObjType::MT_TROOPSHOT)].speed = 10*FRACUNIT; 
    } 
...
{ 
    ...
    *demo_p++ = VERSION;
    *demo_p++ = static_cast<int>(gameskill); 
    *demo_p++ = gameepisode; 
    *demo_p++ = gamemap; 
    *demo_p++ = deathmatch; 
    *demo_p++ = respawnparm;
